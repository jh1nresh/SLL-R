/* SLL-R vendors scroll-world from oso95/scroll-world (MIT).
   License: docs/third-party/scroll-world-LICENSE.txt
   ============================================================================
...
function mountScrollWorld(container, config) {
  ...
  SECTIONS.forEach((s, i) => {
    const dive = { kind: 'dive', si: i, clip: s.clip, clipM: s.clipMobile, still: s.still, stillM: s.stillMobile, accent: s.accent,
                   objectPositionMobile: s.objectPositionMobile,
    ...
    if (i < N - 1 && CONNECTORS[i]) {
      SEGMENTS.push({ kind: 'conn', si: i, clip: CONNECTORS[i], clipM: CONNECTORS_M[i],
                      still: SECTIONS[i + 1].still, stillM: SECTIONS[i + 1].stillMobile, accent: SECTIONS[i + 1].accent,
                      objectPositionMobile: SECTIONS[i + 1].objectPositionMobile, w: CONN_W });
    }
  });
  ...
  route.setAttribute('aria-label', 'Journey sections');
  ...
  SEGMENTS.forEach(s => {
    ...
    scene.style.setProperty('--sw-object-position-mobile', s.objectPositionMobile || 'center 44%');
    const img = el('img', 'sw-scene__still'); img.alt = ''; img.decoding = 'async'; img.loading = 'lazy';
    if (s.still) img.src = (isMobile() && s.stillM) ? s.stillM : s.still;
  ...
  SECTIONS.forEach((s, i) => {
    ...
    c.id = s.id ? `journey-${s.id}` : `journey-${i + 1}`;
    c.innerHTML =
  ...
  function read() {
    ...
    if (near !== activeIndex) {
      ...
      dots.forEach((d, k) => {
        ...
        if (k === near) d.setAttribute('aria-current', 'step'); else d.removeAttribute('aria-current');
      });
      nav.querySelectorAll('.sw-nav__item').forEach((n, k) => {
        n.classList.toggle('is-active', k === near);
        if (k === near) n.setAttribute('aria-current', 'step'); else n.removeAttribute('aria-current');
      });
  ...
  function resumeRaf() {
    if (!document.hidden && container.isConnected && !rafId) rafId = requestAnimationFrame(raf);
  }
  ...
  document.addEventListener('visibilitychange', resumeRaf);
  layout();
  resumeRaf();
...
function injectCSS() {
  ...
  .sw-nav__item:focus-visible{outline:2px solid var(--sw-ink);outline-offset:2px;} .sw-nav__item.is-active{color:#fff;background:var(--sw-accent);}
  ...
  .sw-copylayer::before{content:"";position:absolute;inset:0;width:min(48vw,700px);background:var(--sw-bg);border-right:1px solid color-mix(in srgb,var(--sw-ink) 8%,transparent);}
  ...
  .sw-copy__title{font-family:var(--sw-font-display);font-weight:700;color:var(--sw-ink);font-size:3.5rem;line-height:1.03;margin:12px 0 0;letter-spacing:0;text-shadow:0 2px 20px color-mix(in srgb,var(--sw-bg) 70%,transparent);}
  ...
  .sw-btn:focus-visible,.sw-route__dot:focus-visible{outline:2px solid var(--sw-ink);outline-offset:3px;}
  ...
  .sw-route__dot:focus-visible .sw-route__label,.sw-route__dot.is-active .sw-route__label{opacity:1;transform:translateY(-50%) translateX(0);}
  ...
  @media (max-width:860px){
    .sw-topbar{background:var(--sw-bg);border-bottom:1px solid color-mix(in srgb,var(--sw-ink) 8%,transparent);}
    .sw-nav{display:none;}
    .sw-copylayer::before{width:100%;height:66%;top:auto;bottom:0;background:var(--sw-bg);border-right:0;border-top:1px solid color-mix(in srgb,var(--sw-ink) 8%,transparent);}
    ...
    .sw-copy{left:clamp(18px,5vw,64px);right:clamp(18px,5vw,64px);top:auto;bottom:clamp(150px,20vh,180px);transform:none;width:auto;max-width:560px;}
    .sw-copy{bottom:calc(clamp(150px,20dvh,180px) + env(safe-area-inset-bottom));}
    .sw-copy__title{font-size:2.25rem;}
    .sw-copy__body{max-width:none;font-size:1rem;} .sw-scene__video,.sw-scene__still{object-position:center 46%;}
  ...
  @media (max-width:860px) and (orientation:portrait){
    .sw-scene__video,.sw-scene__still{object-position:var(--sw-object-position-mobile);}
  }
  @media (max-width:420px) and (max-height:760px){
    .sw-copy{bottom:calc(42px + env(safe-area-inset-bottom));}
    .sw-copy__eyebrow{margin-top:10px;}
    .sw-copy__title{font-size:1.9rem;margin-top:8px;}
    .sw-copy__body{margin-top:10px;line-height:1.42;}
    .sw-copy__tags{margin-top:12px;gap:6px;}
    .sw-copy__tags li{padding:5px 9px;font-size:.74rem;}
    .sw-copy__cta{margin-top:14px;gap:8px;}
    .sw-btn{padding:11px 14px;font-size:.86rem;}
  }
  ...
  @media (hover:hover) and (pointer:fine){
    ...
    .sw-route__dot:hover .sw-route__label{opacity:1;transform:translateY(-50%) translateX(0);}
  }
